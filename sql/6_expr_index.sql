...
BEGIN
    CREATE INDEX name_expression_index ON indexes (floor(name));
    ...
    WHILE i <= 1000000 LOOP
        ...
        -- 3 SELECT
        start_time := clock_timestamp();
        EXECUTE format('SELECT name FROM indexes WHERE name = %s', floor(random()*1000000)+1);
        EXECUTE format('SELECT name FROM indexes WHERE name = %s', floor(random()*1000000)+1);
        EXECUTE format('SELECT name FROM indexes WHERE name = %s', floor(random()*1000000)+1);
        end_time := clock_timestamp();
        ...
        -- 3 UPDATE
        start_time := clock_timestamp();
        EXECUTE format('UPDATE indexes SET action = floor(random()*3)+1 WHERE name = %s', floor(random()*1000000)+1);
        EXECUTE format('UPDATE indexes SET action = floor(random()*3)+1 WHERE name = %s', floor(random()*1000000)+1);
        EXECUTE format('UPDATE indexes SET action = floor(random()*3)+1 WHERE name = %s', floor(random()*1000000)+1);
        end_time := clock_timestamp();
